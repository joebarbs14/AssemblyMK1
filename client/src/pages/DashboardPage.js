...
const getCategoryIcon = (category) => {
  ...
  switch (category) {
    ...
    case 'Community News':
      return (
        <svg style={iconStyle} viewBox="0 0 24 24">
          <path d="M21 15h-9v-2h9v2zm0-4h-9V9h9v2zm0-4h-9V5h9v2zm-9 12h-9v-2h9v2zm0-4h-9v-2h9v2zm0-4h-9V9h9v2zM3 3h9v2H3V3z" />
        </svg>
      );
...
function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (storedUserCouncilName) setUserCouncilName(storedUserCouncilName);
  ...
  const handleRoadSubTileClick = (subCategory) => setSelectedRoadSubCategory(subCategory);
  ...
  const renderCommunityDetails = () => {
    if (!selectedCommunitySubCategory) {
      return <p className="no-entries">Please select a community sub-category.</p>;
    }
    return (
  ...
  const renderEnvironmentContent = () => (
    <>
      ...
      <div className="environment-sub-category-details">
        {selectedEnvironmentSubCategory ? (
          <p className="no-entries">No entries found for {selectedEnvironmentSubCategory}.</p>
        ) : (
                    ...
                    <li key={item.id} className="process-item-full-detail">
                      <>
                        ...
                        {item.form_data && Object.keys(item.form_data).length > 0 && (
                          <div className="process-detail">Form Data: {JSON.stringify(item.form_data)}</div>
                        )}
                      </>
                    </li>
                  ))}
